import React from "react";

const News = () => {
  return (
    <div className="overflow-hidden bg-[#fcfefe] flex flex-col w-full">
 <div className="bg-[#fcfefe] flex flex-row justify-between h-20 shrink-0 items-center px-48">
        <div className="text-4xl font-['Yeseva_One'] uppercase text-[#1f2b6c]">
          F<div className="text-[#159eec] contents">Blood</div>
        </div>
        <div className="flex flex-row justify-between w-3/5 items-center">
          <div className="flex flex-row gap-1 items-center">
            <img
              src="https://file.rendit.io/n/HdPLsEkJk6cA0F4Ffay7.svg"
              className="min-h-0 min-w-0 w-10 shrink-0"
            />
            <div className="flex flex-col mb-px gap-px w-[140px]">
              <div className="font-['Work_Sans'] font-medium uppercase text-[#1f2b6c] mr-10">
                Emergency
              </div>
              <div className="whitespace-nowrap font-['Work_Sans'] font-medium text-[#159eec]">
                (+84) 919-730-83
              </div>
            </div>
          </div>
          <div className="flex flex-row gap-2 items-center">
            <img
              src="https://file.rendit.io/n/aqEwQUR8fzOUbgrF7QvV.svg"
              className="min-h-0 min-w-0 w-8 shrink-0"
            />
            <div className="flex flex-col gap-px w-48 items-start">
              <div className="whitespace-nowrap font-['Work_Sans'] font-medium uppercase text-[#1f2b6c] w-1/2">
                Work Hour
              </div>
              <div className="whitespace-nowrap font-['Work_Sans'] font-medium text-[#159eec] self-stretch">
                09:00 - 20:00 Everyday
              </div>
            </div>
          </div>
          <div className="flex flex-row gap-2 items-center">
            <img
              src="https://file.rendit.io/n/fyuTtiJurAL2aGLUP6CD.svg"
              className="min-h-0 min-w-0 w-8 shrink-0"
            />
            <div className="flex flex-col gap-px w-32 items-start">
              <div className="font-['Work_Sans'] font-medium uppercase text-[#1f2b6c] w-3/5">
                Location
              </div>
              <div className="whitespace-nowrap font-['Work_Sans'] font-medium text-[#159eec] self-stretch">
                FptSoftware
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="bg-[url(https://file.rendit.io/n/Uv239y8oo4cLPtaYHygg.png)] bg-cover bg-50%_50% bg-blend-normal bg-no-repeat flex flex-col mb-16">
        <div className="bg-white/50 flex flex-row items-end">
          <div className="flex flex-col gap-24 w-[1143px] h-64">
            <div className="flex flex-row gap-16 items-end">
              <img
                src="https://file.rendit.io/n/CqjPESKWoKy2JhYgCnl5.svg"
                className="min-h-0 min-w-0 mb-8"
              />
              <div className="relative flex flex-col pb-12 w-64 shrink-0 items-start">
                <div className="whitespace-nowrap text-5xl font-['Yeseva_One'] text-[#1f2b6c] absolute top-5 left-0 h-12 w-64">
                  Blog Posts
                </div>
                <div className="whitespace-nowrap text-lg font-['Work_Sans'] leading-[25.2px] text-[#1f2b6c] relative w-1/2">
                  Home / News
                </div>
              </div>
            </div>
            <div className="relative flex flex-col items-start">
              <div className="w-[837px] h-2 bg-[#1f2b6c] absolute top-0 left-[305.697509765625px]" />
              <div className="bg-[#bfd2f8] relative w-[306px] h-2 shrink-0" />
            </div>
          </div>
          <div className="relative flex flex-col justify-end pt-32 w-56 shrink-0">
            <img
              src="https://file.rendit.io/n/hekRLO3MDiT9ibcGowDH.svg"
              className="w-32 h-32 min-h-0 min-w-0 absolute top-0 left-24"
            />
            <div className="bg-[#159eec] relative h-2 shrink-0" />
          </div>
        </div>
      </div>
      <div className="flex flex-row gap-5 items-start mb-1 mx-48">
        <div className="flex flex-col gap-2 w-[659px] items-center">
          <img
            src="https://file.rendit.io/n/wefIk0PeZLDzHto8PHJe.png"
            className="min-h-0 min-w-0 mb-4"
          />
          <div className="flex flex-row mr-40 gap-2 items-center">
            <img
              src="https://file.rendit.io/n/QYzmA7VJnAaxwvV0GJqf.svg"
              className="min-h-0 min-w-0 w-4 shrink-0"
            />
            <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] mr-4 w-2/5">
              Monday 05, September 2021
            </div>
            <img
              src="https://file.rendit.io/n/dxt3lHIIulZFi2bAbT4I.svg"
              className="min-h-0 min-w-0 w-4 shrink-0"
            />
            <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] mr-4 w-20 shrink-0">
              By Author
            </div>
            <img
              src="https://file.rendit.io/n/BXFnAfpvba5KcubwTJim.svg"
              className="min-h-0 min-w-0 mr-0 w-4 shrink-0"
            />
            <div className="font-['Work_Sans'] leading-[22.4px] mr-4 w-5 shrink-0">
              68
            </div>
            <img
              src="https://file.rendit.io/n/c7QFHiP1Mso5D7F1z5U5.svg"
              className="min-h-0 min-w-0 mr-px w-4 shrink-0"
            />
            <div className="font-['Work_Sans'] leading-[22.4px] w-5 shrink-0">
              86
            </div>
          </div>
          <div className="whitespace-nowrap text-3xl font-['Yeseva_One'] text-[#1f2b6c] mb-2 ml-px mr-20">
            A passion for putting patients first
          </div>
          <div className="font-['Work_Sans'] leading-[22.4px] text-[#212124] ml-px">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque
            placerat scelerisque tortor ornare ornare. Quisque placerat
            scelerisque tortor ornare ornare Convallis felis vitae tortor augue.
            Velit nascetur proin massa in. Consequat faucibus porttitor enim
            et....
          </div>
        </div>
        <div className="flex flex-col gap-6">
          <div className="bg-[#1f2b6c] flex flex-row justify-between h-12 shrink-0 items-center px-5 rounded">
            <div className="font-['Work_Sans'] leading-[22.4px] text-[#fcfefe] w-12 shrink-0">
              Search
            </div>
            <img
              src="https://file.rendit.io/n/pLjFH8UoI7wQTvCENDWL.svg"
              className="min-h-0 min-w-0 w-5 shrink-0"
            />
          </div>
          <div className="border-solid border-[#1f2b6c] bg-[#fcfefe] flex flex-col justify-center gap-6 p-5 border rounded">
            <div className="whitespace-nowrap text-3xl font-['Yeseva_One'] text-[#1f2b6c] mr-16">
              Recent Posts
            </div>
            <div className="flex flex-col justify-between gap-2">
              <div className="flex flex-row gap-2 items-center">
                <img
                  src="https://file.rendit.io/n/hHXXsDP03wTP0s2F8K1n.png"
                  className="min-h-0 min-w-0 w-16 shrink-0"
                />
                <div className="flex flex-col gap-1 w-[204px]">
                  <div className="whitespace-nowrap text-xs font-['Work_Sans'] text-[#159eec] mr-10">
                    Monday 05, September 2021
                  </div>
                  <div className="whitespace-nowrap text-sm font-['Work_Sans'] font-medium text-[#212124]">
                    This Article’s Title goes Here, <br />
                    but not too long.
                  </div>
                </div>
              </div>
              <div className="flex flex-row gap-2 items-center">
                <img
                  src="https://file.rendit.io/n/gAyH2GyMhhKAIMFs6wIE.png"
                  className="min-h-0 min-w-0 w-16 shrink-0"
                />
                <div className="flex flex-col gap-1 w-[204px]">
                  <div className="whitespace-nowrap text-xs font-['Work_Sans'] text-[#159eec] mr-10">
                    Monday 05, September 2021
                  </div>
                  <div className="whitespace-nowrap text-sm font-['Work_Sans'] font-medium text-[#212124]">
                    This Article’s Title goes Here, <br />
                    but not too long.
                  </div>
                </div>
              </div>
              <div className="flex flex-row gap-2 items-center">
                <img
                  src="https://file.rendit.io/n/FX8XSDF0ychAGiRDdD8u.png"
                  className="min-h-0 min-w-0 w-16 shrink-0"
                />
                <div className="flex flex-col gap-1 w-[204px]">
                  <div className="whitespace-nowrap text-xs font-['Work_Sans'] text-[#159eec] mr-10">
                    Monday 05, September 2021
                  </div>
                  <div className="whitespace-nowrap text-sm font-['Work_Sans'] font-medium text-[#212124]">
                    This Article’s Title goes Here, <br />
                    but not too long.
                  </div>
                </div>
              </div>
              <div className="flex flex-row gap-2 items-center">
                <img
                  src="https://file.rendit.io/n/dCi0qasLUNucXlVI9faF.png"
                  className="min-h-0 min-w-0 w-16 shrink-0"
                />
                <div className="flex flex-col gap-1 w-[204px]">
                  <div className="whitespace-nowrap text-xs font-['Work_Sans'] text-[#159eec] mr-10">
                    Monday 05, September 2021
                  </div>
                  <div className="whitespace-nowrap text-sm font-['Work_Sans'] font-medium text-[#212124]">
                    This Article’s Title goes Here, <br />
                    but not too long.
                  </div>
                </div>
              </div>
              <div className="flex flex-row gap-2 items-center">
                <img
                  src="https://file.rendit.io/n/ZP091AzT2qof5OiiXV1y.png"
                  className="min-h-0 min-w-0 w-16 shrink-0"
                />
                <div className="flex flex-col gap-1 w-[204px]">
                  <div className="whitespace-nowrap text-xs font-['Work_Sans'] text-[#159eec] mr-10">
                    Monday 05, September 2021
                  </div>
                  <div className="whitespace-nowrap text-sm font-['Work_Sans'] font-medium text-[#212124]">
                    This Article’s Title goes Here, <br />
                    but not too long.
                  </div>
                </div>
              </div>
              <div className="flex flex-row gap-2 items-center">
                <img
                  src="https://file.rendit.io/n/O30kDrrfvT3lUqHb64LT.png"
                  className="min-h-0 min-w-0 w-16 shrink-0"
                />
                <div className="flex flex-col gap-1 w-[204px]">
                  <div className="whitespace-nowrap text-xs font-['Work_Sans'] text-[#159eec] mr-10">
                    Monday 05, September 2021
                  </div>
                  <div className="whitespace-nowrap text-sm font-['Work_Sans'] font-medium text-[#212124]">
                    This Article’s Title goes Here, <br />
                    but not too long.
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="flex flex-row gap-5 items-end mb-6 mx-48">
        <div className="flex flex-col mt-5 gap-8 w-[658px] items-start">
          <button className="bg-[#bfd2f8] flex flex-row justify-center gap-2 h-12 shrink-0 items-center px-6 py-3 rounded-[50px]">
            <div className="whitespace-nowrap font-['Work_Sans'] font-medium text-[#1f2b6c] w-4/5">
              Read More
            </div>
            <img
              src="https://file.rendit.io/n/E5DPfUoLLmsTwyqPGIPw.svg"
              className="min-h-0 min-w-0 w-3 shrink-0"
            />
          </button>
          <img
            src="https://file.rendit.io/n/ss8ZyZx230FozqdsTk0m.png"
            className="min-h-0 min-w-0"
          />
        </div>
        <div className="border-solid border-[#1f2b6c] bg-[#fcfefe] flex flex-col justify-center gap-6 p-5 border rounded">
          <div className="text-3xl font-['Yeseva_One'] text-[#1f2b6c] w-2/3">
            Categories
          </div>
          <div className="flex flex-col">
            <div className="bg-[#fcfefe] flex flex-row justify-between h-12 shrink-0 items-center px-5 rounded">
              <div className="text-lg font-['Work_Sans'] leading-[25.2px] text-[#212124] w-16 shrink-0">
                Surgery
              </div>
              <div className="bg-[#159eec] flex flex-col w-6 shrink-0 items-center px-2 py-px rounded-[50px]">
                <div className="font-['Work_Sans'] leading-[22.4px] text-[#fcfefe] w-full">
                  3
                </div>
              </div>
            </div>
            <div className="bg-[#fcfefe] flex flex-row justify-between h-12 shrink-0 items-center px-5 rounded">
              <div className="whitespace-nowrap text-lg font-['Work_Sans'] leading-[25.2px] text-[#212124] w-24 shrink-0">
                Health Care
              </div>
              <div className="bg-[#159eec] flex flex-col w-6 shrink-0 items-center px-2 py-px rounded-[50px]">
                <div className="font-['Work_Sans'] leading-[22.4px] text-[#fcfefe] w-full">
                  5
                </div>
              </div>
            </div>
            <div className="bg-[#fcfefe] flex flex-row justify-between h-12 shrink-0 items-center px-5 rounded">
              <div className="text-lg font-['Work_Sans'] leading-[25.2px] text-[#212124] w-16 shrink-0">
                Medical
              </div>
              <div className="bg-[#159eec] flex flex-col w-6 shrink-0 items-center px-2 py-px rounded-[50px]">
                <div className="font-['Work_Sans'] leading-[22.4px] text-[#fcfefe] w-full">
                  8
                </div>
              </div>
            </div>
            <div className="bg-[#fcfefe] flex flex-row justify-center gap-24 h-12 shrink-0 items-center rounded">
              <div className="text-lg font-['Work_Sans'] leading-[25.2px] text-[#212124] w-[108px] shrink-0">
                Professional
              </div>
              <div className="bg-[#159eec] flex flex-col w-8 shrink-0 items-center px-2 py-px rounded-[50px]">
                <div className="font-['Work_Sans'] leading-[22.4px] text-[#fcfefe] w-full">
                  10
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="flex flex-row gap-2 items-center mb-2 ml-48">
        <img
          src="https://file.rendit.io/n/QYzmA7VJnAaxwvV0GJqf.svg"
          className="min-h-0 min-w-0 w-4 shrink-0"
        />
        <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] mr-4 w-2/5">
          Monday 05, September 2021
        </div>
        <img
          src="https://file.rendit.io/n/dxt3lHIIulZFi2bAbT4I.svg"
          className="min-h-0 min-w-0 w-4 shrink-0"
        />
        <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] mr-4 w-20 shrink-0">
          By Author
        </div>
        <img
          src="https://file.rendit.io/n/BXFnAfpvba5KcubwTJim.svg"
          className="min-h-0 min-w-0 mr-0 w-4 shrink-0"
        />
        <div className="font-['Work_Sans'] leading-[22.4px] mr-4 w-5 shrink-0">
          68
        </div>
        <img
          src="https://file.rendit.io/n/c7QFHiP1Mso5D7F1z5U5.svg"
          className="min-h-0 min-w-0 mr-px w-4 shrink-0"
        />
        <div className="font-['Work_Sans'] leading-[22.4px] w-5 shrink-0">
          86
        </div>
      </div>
      <div className="whitespace-nowrap text-3xl font-['Yeseva_One'] text-[#1f2b6c] w-2/5 mb-4 ml-48">
        A passion for putting patients first
      </div>
      <div className="font-['Work_Sans'] leading-[22.4px] text-[#212124] w-1/2 mb-6 ml-48">
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque
        placerat scelerisque tortor ornare ornare. Quisque placerat scelerisque
        tortor ornare ornare Convallis felis vitae tortor augue. Velit nascetur
        proin massa in. Consequat faucibus porttitor enim et....
      </div>
      <button className="bg-[#bfd2f8] flex flex-row justify-center gap-2 h-12 shrink-0 items-center mb-8 ml-48 px-6 py-3 rounded-[50px]">
        <div className="whitespace-nowrap font-['Work_Sans'] font-medium text-[#1f2b6c] w-4/5">
          Read More
        </div>
        <img
          src="https://file.rendit.io/n/E5DPfUoLLmsTwyqPGIPw.svg"
          className="min-h-0 min-w-0 w-3 shrink-0"
        />
      </button>
      <img
        src="https://file.rendit.io/n/cphshGdExThbjbLonGOV.png"
        className="min-h-0 min-w-0 mb-6 ml-48"
      />
      <div className="flex flex-row gap-2 items-center mb-2 ml-48">
        <img
          src="https://file.rendit.io/n/XiLE5pzn4edxKl6jRy3X.svg"
          className="min-h-0 min-w-0 w-4 shrink-0"
        />
        <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] mr-4 w-2/5">
          Monday 05, September 2021
        </div>
        <img
          src="https://file.rendit.io/n/dxt3lHIIulZFi2bAbT4I.svg"
          className="min-h-0 min-w-0 w-4 shrink-0"
        />
        <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] mr-4 w-20 shrink-0">
          By Author
        </div>
        <img
          src="https://file.rendit.io/n/BXFnAfpvba5KcubwTJim.svg"
          className="min-h-0 min-w-0 mr-0 w-4 shrink-0"
        />
        <div className="font-['Work_Sans'] leading-[22.4px] mr-4 w-5 shrink-0">
          68
        </div>
        <img
          src="https://file.rendit.io/n/c7QFHiP1Mso5D7F1z5U5.svg"
          className="min-h-0 min-w-0 mr-px w-4 shrink-0"
        />
        <div className="font-['Work_Sans'] leading-[22.4px] w-5 shrink-0">
          86
        </div>
      </div>
      <div className="whitespace-nowrap text-3xl font-['Yeseva_One'] text-[#1f2b6c] w-2/5 mb-4 ml-48">
        A passion for putting patients first
      </div>
      <div className="font-['Work_Sans'] leading-[22.4px] text-[#212124] w-1/2 mb-6 ml-48">
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque
        placerat scelerisque tortor ornare ornare. Quisque placerat scelerisque
        tortor ornare ornare Convallis felis vitae tortor augue. Velit nascetur
        proin massa in. Consequat faucibus porttitor enim et....
      </div>
      <button className="bg-[#bfd2f8] flex flex-row justify-center gap-2 h-12 shrink-0 items-center mb-8 ml-48 px-6 py-3 rounded-[50px]">
        <div className="whitespace-nowrap font-['Work_Sans'] font-medium text-[#1f2b6c] w-4/5">
          Read More
        </div>
        <img
          src="https://file.rendit.io/n/E5DPfUoLLmsTwyqPGIPw.svg"
          className="min-h-0 min-w-0 w-3 shrink-0"
        />
      </button>
      <img
        src="https://file.rendit.io/n/2tnsLej86C2g9CtzW9cu.png"
        className="min-h-0 min-w-0 mb-6 ml-48"
      />
      <div className="flex flex-row gap-2 items-center mb-2 ml-48">
        <img
          src="https://file.rendit.io/n/yrkk9TJ4Qov2CYpcVpKh.svg"
          className="min-h-0 min-w-0 w-4 shrink-0"
        />
        <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] mr-4 w-2/5">
          Monday 05, September 2021
        </div>
        <img
          src="https://file.rendit.io/n/dxt3lHIIulZFi2bAbT4I.svg"
          className="min-h-0 min-w-0 w-4 shrink-0"
        />
        <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] mr-4 w-20 shrink-0">
          By Author
        </div>
        <img
          src="https://file.rendit.io/n/BXFnAfpvba5KcubwTJim.svg"
          className="min-h-0 min-w-0 mr-0 w-4 shrink-0"
        />
        <div className="font-['Work_Sans'] leading-[22.4px] mr-4 w-5 shrink-0">
          68
        </div>
        <img
          src="https://file.rendit.io/n/c7QFHiP1Mso5D7F1z5U5.svg"
          className="min-h-0 min-w-0 mr-px w-4 shrink-0"
        />
        <div className="font-['Work_Sans'] leading-[22.4px] w-5 shrink-0">
          86
        </div>
      </div>
      <div className="whitespace-nowrap text-3xl font-['Yeseva_One'] text-[#1f2b6c] w-2/5 mb-4 ml-48">
        A passion for putting patients first
      </div>
      <div className="font-['Work_Sans'] leading-[22.4px] text-[#212124] w-1/2 mb-6 ml-48">
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque
        placerat scelerisque tortor ornare ornare. Quisque placerat scelerisque
        tortor ornare ornare Convallis felis vitae tortor augue. Velit nascetur
        proin massa in. Consequat faucibus porttitor enim et....
      </div>
      <button className="bg-[#bfd2f8] flex flex-row justify-center gap-2 h-12 shrink-0 items-center mb-8 ml-48 px-6 py-3 rounded-[50px]">
        <div className="whitespace-nowrap font-['Work_Sans'] font-medium text-[#1f2b6c] w-4/5">
          Read More
        </div>
        <img
          src="https://file.rendit.io/n/E5DPfUoLLmsTwyqPGIPw.svg"
          className="min-h-0 min-w-0 w-3 shrink-0"
        />
      </button>
      <div className="flex flex-row gap-40 w-1/2 items-center mb-16 ml-48">
        <div className="relative flex flex-col w-32 shrink-0 items-start py-1">
          <button className="w-32 h-5 absolute top-0 left-px flex flex-col">
            <button className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#159eec] ml-5">
              Previous Page
            </button>
          </button>
          <img
            src="https://file.rendit.io/n/QX2QdAVrvU2n1JG8BgE4.svg"
            className="min-h-0 min-w-0 relative w-3"
          />
        </div>
        <div className="text-right whitespace-nowrap font-['Work_Sans'] leading-[22.4px] mr-px">
          1 - <div className="text-[#159eec] contents">2 </div>
          <div className="contents">-</div>
          <div className="text-[#159eec] contents"> 3 </div>
          <div className="contents">-</div>
          <div className="text-[#159eec] contents"> 4 </div>
          <div className="contents">-</div>
          <div className="text-[#159eec] contents"> 5</div>
        </div>
        <div className="relative flex flex-col w-24 shrink-0 items-end py-1">
          <button className="w-24 h-5 absolute top-0 left-0 flex flex-col">
            <button className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#159eec] mr-5">
              Next Page
            </button>
          </button>
          <img
            src="https://file.rendit.io/n/Xv8spOd7CG2IOGuTlGKE.svg"
            className="min-h-0 min-w-0 relative w-3"
          />
        </div>
      </div>
      <div className="text-center whitespace-nowrap text-lg font-['Work_Sans'] font-bold tracking-[2.88] uppercase text-[#159eec] self-center mb-2 w-40">
        Get in touch
      </div>
      <div className="text-center text-3xl font-['Yeseva_One'] text-[#1f2b6c] self-center mb-16 w-32">
        Contact
      </div>
      <div className="flex flex-row justify-between items-center mb-12 mx-48">
        <div className="bg-[#bfd2f8] flex flex-col gap-1 h-56 items-start pl-6 py-12 rounded">
          <img
            src="https://file.rendit.io/n/bagk2cFLAg1QuqaneHEJ.svg"
            className="min-h-0 min-w-0 mb-2 w-10"
          />
          <div className="text-lg font-['Work_Sans'] font-bold uppercase text-[#1f2b6c] w-1/2 mb-1 ml-1">
            Emergency
          </div>
          <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#1f2b6c] ml-1 w-2/3">
            (237) 681-812-255
          </div>
          <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#1f2b6c] w-2/3 ml-1 mr-16">
            (237) 666-331-894
          </div>
        </div>
        <div className="bg-[#1f2b6c] flex flex-col gap-1 h-56 px-8 py-12 rounded">
          <img
            src="https://file.rendit.io/n/hEeeAKT1Po8tsm0ZyEBK.svg"
            className="min-h-0 min-w-0 mb-2 w-8"
          />
          <div className="text-lg font-['Work_Sans'] font-bold uppercase text-[#bfd2f8] mb-1 w-24">
            Location
          </div>
          <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#bfd2f8] w-3/4">
            0123 Some place
          </div>
          <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#bfd2f8] mr-5">
            9876 Some country
          </div>
        </div>
        <div className="bg-[#bfd2f8] flex flex-col justify-center gap-4 h-56 px-5 rounded">
          <img
            src="https://file.rendit.io/n/sfknWJXcekkMxEK0UTPT.svg"
            className="min-h-0 min-w-0 ml-2 w-10"
          />
          <div className="flex flex-col ml-2 gap-1 items-start">
            <div className="text-lg font-['Work_Sans'] font-bold uppercase text-[#1f2b6c] mb-1 w-16">
              Email
            </div>
            <div className="font-['Work_Sans'] leading-[22.4px] text-[#1f2b6c]">
              fildineeesoe@gmil.com
            </div>
            <div className="text-sm font-['Work_Sans'] text-[#1f2b6c] mr-2">
              myebstudios@gmail.com
            </div>
          </div>
        </div>
        <div className="bg-[#bfd2f8] flex flex-col justify-center gap-4 h-56 px-4 rounded">
          <img
            src="https://file.rendit.io/n/FczxrBEc2eNGORUgpMp4.svg"
            className="min-h-0 min-w-0 ml-3 w-8"
          />
          <div className="flex flex-col ml-3 gap-1">
            <div className="whitespace-nowrap text-lg font-['Work_Sans'] font-bold uppercase text-[#1f2b6c] mb-1 mr-6">
              Working Hours
            </div>
            <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#1f2b6c] mr-4">
              Mon-Sat 09:00-20:00
            </div>
            <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#1f2b6c]">
              Sunday Emergency only
            </div>
          </div>
        </div>
      </div>
      <div className="bg-[#1f2b6c] flex flex-col gap-1 h-[427px] shrink-0 pt-16 pb-20 px-48">
        <div className="flex flex-row justify-between mb-3 w-5/6 items-start">
          <div className="flex flex-row gap-20 w-[548px] items-start">
            <div className="text-4xl font-['Yeseva_One'] uppercase text-[#bfd2f8] w-48 shrink-0">
              Meddical
            </div>
            <div className="flex flex-row gap-8 w-1/2 items-center">
              <div className="whitespace-nowrap text-lg font-['Work_Sans'] font-semibold text-[#fcfefe] mb-px w-1/2">
                Important Links
              </div>
              <div className="whitespace-nowrap text-lg font-['Work_Sans'] font-semibold text-[#fcfefe] w-24 shrink-0">
                Contact Us
              </div>
            </div>
          </div>
          <div className="whitespace-nowrap text-lg font-['Work_Sans'] font-semibold text-[#fcfefe] w-24 shrink-0">
            News letter
          </div>
        </div>
        <div className="flex flex-row mr-px gap-20 items-center">
          <div className="flex flex-row gap-8 items-center">
            <div className="text-lg font-['Work_Sans'] leading-[25.2px] text-[#fcfefe] w-2/3">
              Leading the Way in Medical
              <br />
              Execellence, Trusted Care.
            </div>
            <div className="flex flex-col gap-2">
              <div className="font-['Work_Sans'] leading-[22.4px] text-[#fcfefe]">
                Appointment
              </div>
              <div className="font-['Work_Sans'] leading-[22.4px] text-[#fcfefe] w-3/5">
                Doctors
              </div>
            </div>
          </div>
          <div className="flex flex-row gap-8 w-1/2 items-center">
            <div className="flex flex-col mt-px gap-2 w-2/5">
              <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#fcfefe] mr-12">
                Call: (237) 681-812-255
              </div>
              <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#fcfefe]">
                Email: fildineesoe@gmail.com
              </div>
            </div>
            <div className="bg-[#bfd2f8] flex flex-row gap-6 w-1/2 h-12 items-center px-2 rounded">
              <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#1f2b6c] w-48">
                Enter your email address
              </div>
              <img
                src="https://file.rendit.io/n/N2JkpCGstgpRjkDTKgKb.svg"
                className="min-h-0 min-w-0 w-6 shrink-0"
              />
            </div>
          </div>
        </div>
        <div className="flex flex-row justify-between ml-[275px] gap-[109px] items-center">
          <div className="font-['Work_Sans'] leading-[22.4px] text-[#fcfefe] mb-px w-16 shrink-0">
            Services
          </div>
          <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#fcfefe] w-1/2">
            Address: 0123 Some place
          </div>
        </div>
        <div className="flex flex-row justify-between gap-24 items-center mb-10 ml-[275px]">
          <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#fcfefe] mb-px w-16 shrink-0">
            About Us
          </div>
          <div className="whitespace-nowrap font-['Work_Sans'] leading-[22.4px] text-[#fcfefe] w-[110px] shrink-0">
            Some country
          </div>
        </div>
        <img
          src="https://file.rendit.io/n/GgiMAtQP0iiEVJ0Y9xN2.svg"
          className="min-h-0 min-w-0 self-center mb-10"
        />
        <div className="flex flex-row justify-between items-center">
          <div className="whitespace-nowrap font-['Work_Sans'] text-[#fcfefe] w-1/2">
            © 2021 Hospital’s name All Rights Reserved by PNTEC-LTD
          </div>
          <div className="flex flex-row justify-between gap-5 items-center">
            <img
              src="https://file.rendit.io/n/xumdC8SeEn2S4ybdsc8j.svg"
              className="min-h-0 min-w-0 w-6 shrink-0"
            />
            <img
              src="https://file.rendit.io/n/7oEDwcaVlHpjnGqv0j0a.svg"
              className="min-h-0 min-w-0 w-6 shrink-0"
            />
            <img
              src="https://file.rendit.io/n/Rl2qUUNNGBB0fD9Sg5ZD.svg"
              className="min-h-0 min-w-0 w-6 shrink-0"
            />
          </div>
        </div>
      </div>
    </div>
  );
};

export default News;
